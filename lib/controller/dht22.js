 ...
 * @api private
 */
var save = function(self, temp, humidity, callback) {
  ...
  data['temperature.'+ hour + '.' + minute] = parseFloat(temp);
  data['humidity.'+ hour + '.' + minute] = parseFloat(humidity);
  ...
  query = function() {
    ...
    if(isValidRead(readout.temperature, readout.humidity)) { // Valid result
      ...
      if(self.settings.database === true) {
        ...
        save(self, readout.temperature, readout.humidity, function(err) {
          if(err) {
